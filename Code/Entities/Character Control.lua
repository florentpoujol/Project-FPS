...
function Behavior:Update()
    ...
    elseif IsServer(true) and self.frameCount % 5 == 0 then
        local playerData = {
    ...
    if 
        not input.isNull  -- false when there is no input
        or not lastInputIsNull -- no input but player was still moving the last frame (placing this here only work in local offline)
    then
        self:Move( input )
...
function Behavior:Move( input )
    ...
    if self.lookAngles == nil then
        self.lookAngles = self.gameObject.transform.eulerAngles -- this actually let the character correctly oriented when the first Update() is run
    end
    ...
    local newVelocity = Vector3:Forward() * vertical * self.walkSpeed -- /!\ the speed depends on the framerate ! Booooo
    -- Strafing (left/right)
...
function Behavior:Shoot()
    ...
    if hit.normal ~= nil then -- the map has been hit
        self:SpawnBulletHit( { position = hit.hitLocation } )
    end
end
...
function Behavior:CreateShootLine( data )
    ...
    if endPosition == nil then -- That's the case when the player shoot outside the map
        endPosition = self.shootRay.position + self.shootRay.direction * 9999
    else
        endPosition = Vector3(endPosition)
    end
    
    local shootRayPosition = self.shootRay.position
    if data.shootRayPosition then
        shootRayPosition = Vector3(data.shootRayPosition)
    end
        
    if IsServer(true) then
        self.gameObject.networkSync:SendMessageToPlayers( "CreateShootLine", { endPosition = endPosition, shootRayPosition = shootRayPosition }, LocalServer.playerIds )
    end
    ...
    local length = (endPosition - shootRayPosition):GetLength()
...
function Behavior:SpawnBulletHit( data )
    if IsServer(true) then
        self.gameObject.networkSync:SendMessageToPlayers( "SpawnBulletHit", data, LocalServer.playerIds )
    end
...
CS.Network.RegisterMessageHandler( Behavior.SpawnBulletHit, CS.Network.MessageSide.Players )
