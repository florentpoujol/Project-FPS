...
function Behavior:Awake()
    self.gameObject.transform.position = Vector3(0,-999,0)
    --GUI.Awake()
    ...
    Level.hudCamera = Level.hudCameraGO -- why ?
...
function Behavior:Start()
    ...
    -- Player text
    local payerTextGO = Level.hud:GetChild("Player Text")
    Level.hud.playerText = payerTextGO.textArea
    Level.hud.playerText.text = ""
    
    Level.hud.playerText.AddText = function( textArea, text )
        textArea:AddLine( text )
        local go = textArea.lineGOs[#textArea.lineGOs]
        go:Animate( "opacity", 0, 2, function()
            table.removevalue( textArea.lineGOs, go )
            textArea.Text = textArea.Text:sub( #text + #textArea.NewLine + 1 )
            go:Destroy()
        end )
    end
end
